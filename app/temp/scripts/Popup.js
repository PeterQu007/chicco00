...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _NewPage = __webpack_require__(15);
	...
	var newPage = new _NewPage2.default(0);
	newPage.getTabTitle();
	...
	$(function () {
		...
		$('#getListing').click(function () {
			...
			chrome.tabs.query({
				...
				chrome.tabs.sendMessage(tabs[0].id, { from: 'popup', subject: 'listingInfo' }, setListingInfo);
			});
		});
		...
		$('#repost').click(function () {
			...
			chrome.tabs.query({
				...
				chrome.tabs.sendMessage(tabs[0].id, { from: 'popup', subject: 'repost' }, newPage.getTabtitle); //pass me the next available listing
			});
		});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //Open New Page / Tab of Craigslist
	...
	function setListingInfo(info) {
			...
			console.log('setListingInfo.1.0: ', info);
			//setup the popup.html, show the available listing
			(0, _jquery2.default)('#status').text(info.status);
			(0, _jquery2.default)('#manage').text(info.manage);
			(0, _jquery2.default)('#postdate').text(info.postedDate);
			(0, _jquery2.default)('#title').text(info.title);
	};

	var NewPage = function () {
			function NewPage(debug) {
			...
			_createClass(NewPage, [{
					...
					value: function getTabTitle() {
							...
							chrome.tabs.query({
									...
									chrome.tabs.sendMessage(tabs[0].id, { from: 'popup', subject: 'currentTabTitle' }, function (info) {

											_$('readListing return tabTitle: ', info);

											if (info !== undefined) {
													self.tabTitle = info.tabTitle;
											}
					...
					value: function open() {
							...
							if (self.tabTitle != self.defaultTabTitla) {
									...
									(function () {
											...
											//var newURL = "https://accounts.craigslist.org/login?lang=en&cc=us&rt=L&rp=%2Flogin%2Fhome%3Flang%3Den%26cc%3Dus";
											var newURL = _Pages2.default.cgHomePage;
											chrome.tabs.create({ url: newURL });
									})();
							}
							...
							chrome.tabs.query({
									...
									chrome.tabs.sendMessage(tabs[0].id, { from: 'popup', subject: 'currentListingInfo' }, setListingInfo); //the message will be passed back thru setDOMInfo
							}); // end of query
