...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _Listing = __webpack_require__(6);
	...
	var _PostMenu = __webpack_require__(9);

	var _PostMenu2 = _interopRequireDefault(_PostMenu);

	var _Repost = __webpack_require__(10);
	...
	var _Copy = __webpack_require__(13);
	...
	_$$$("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<new cgManager.js");
	var Debug = { logLocal: 1, logBackground: 2, logBG: 2, logDisable: 3 }; //BG === Background
	...
	backupListing = new _Listing2.default();
	listingInfo = new _Listing2.default();
	pn = new _Paginator2.default('#paginator', 1);
	...
	function checkPage() {
			...
			if (pn.pageChanged !== null) {
					var checkCurrentListing = function checkCurrentListing() {
							...
							if (listingInfo && pn.pageChanged !== null) {
									...
									cgPostMenu = new _PostMenu2.default(tableSelector, 0);
									cgRepost = new _Repost2.default(pn);
									...
									cgDelete = new _Delete2.default(pn, Debug.logDisable);
									cgEdit = new _Edit2.default(pn, Debug.logDisable);

									cgCopy = new _Copy2.default(pn, Debug.logLocal);
							}
					};
	...
	chrome.runtime.onMessage.addListener(function (msg, sender, response) {
			...
			if (msg.from === 'popup' && msg.subject === 'repost') {
					...
					listingInfo.from = "readListing", listingInfo.subject = "backupListing";
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); // function Listing(status, postID, manage, description, postedDate, from, subject){
	//   this.status = status || '';
	//   this.postID = postID || '';
	//   this.manage = manage || '';
	...
	var Debug = { logLocal: 1, logBackground: 2, logBG: 2, disable: 3 }; //BG === Background
	...
	var Listing = function () {
		function Listing(status, postID, manage, description, postedDate, from, subject, debug) {
		...
		_createClass(Listing, [{
			...
			value: function readRow(tr) {
				...
				try {
					...
					var thisRow = (0, _jquery2.default)(tr);
					...
					self.manage = _jquery2.default.map(thisRow.find("td:eq(" + fields.manage + ")").find('input[type="submit"]'), function (value) {
						return value.value;
					});
			...
			key: "readTitleAndPrice",
			value: function readTitleAndPrice(description) {
			...
			key: "readAreaAndCat",
			value: function readAreaAndCat(areaAndCat) {
			...
			key: "addForm",
			value: function addForm(name, form) {
			...
			key: "edit",
			value: function edit(price, debug) {
				if (debug) {
			...
			key: "saveToFav",
			value: function saveToFav() {
				this.forms.saveForm.submit();
			}
		}, {
			key: "removeFromFav",
			value: function removeFromFav() {
				this.forms.removeForm.submit();
			}
		}, {
			key: "$$$",
			value: function $$$(dbg, msg) {
				...
				switch (debug) {
					...
					case Debug.logBackground:
						chrome.runtime.sendMessage({ from: 'clsDelete', subject: 'logMessage', log: arguments }, function (response) {
							//to do sth.
						});
						break;
					case Debug.disable:
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //Table class
	...
	var Debug = { logLocal: 1, logBackground: 2, logBG: 2, disable: 3 };
	...
	var Table = function () {
		function Table(tableSelector, debug) {
			...
			this.table = (0, _jquery2.default)(this.tableSelector); //convert the tabelSelector to jquery table element
			this.rows = (0, _jquery2.default)((0, _jquery2.default)(this.tableSelector + ' tbody tr').get().reverse()); //get all of the rows in the table page 1/2/3...
			this.listings = _jquery2.default.map(this.rows, function (row) {
				return new _Listing2.default({ tr: row, debug: Debug.disable });
			});
			//_$(this.listings);
		...
		_createClass(Table, [{
			...
			value: function events() {
				...
				self.checkboxes.click(function () {
					...
					(0, _jquery2.default)(self.currentListing.tr).addClass('currentRow');

					chrome.runtime.sendMessage({ from: 'classTable', subject: 'currentListingUpdated' }, function (response) {
			...
			value: function addCheckboxes() {
				...
				var newHeadTag = '<th class="tablesorter-header ' + 'tablesorter-headerUnSorted ui-widget-header ui-corner-all ui-state-default">' + '<div>select!</div>' + "</th>";
				var newCellTagTemplate = '<td >' + '<input type="checkbox" name="selectPost" class="status Z" value="spost">' + "</td>";
				...
				try {
					...
					(0, _jquery2.default)(newHeadTag).insertAfter(this.tableSelector + " thead tr th:first");

					rows.each(function (index) {
						(0, _jquery2.default)(this).attr('id', 'row' + index);
					});
					...
					rows.each(function (index) {
						...
						_$("the row: ", this);
						(0, _jquery2.default)(newCellTag).insertAfter("#row" + index + " td:first");
					});

					self.checkboxes = (0, _jquery2.default)(self.tableSelector + ' tbody input[type = checkbox]');
			...
			key: 'getBackupListing',
			value: function getBackupListing() {
			...
			key: 'repostableListings',
			value: function repostableListings() {
				//filter the repostable listings 
				return this.rows.filter(function (index) {
					return _jquery2.default.map((0, _jquery2.default)(this).find("td:eq(1)").find('input[type="submit"]'), function (value) {
						return value.value;
					}) == 'repost';
				});
			}
		}, {
			key: 'setCurrentListing',
			value: function setCurrentListing(listing) {

				var self = this;
				var _$ = this.log;

				self.currentListing = (0, _jquery2.default)(self.listings).filter(function () {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //Paginator
	...
	var Paginator = function () {
		function Paginator(paginatorSelector, debug) {
			...
			this.paginator = (0, _jquery2.default)(this.selector).children('#paginator1');
			...
			this.cgTable = new _Table2.default(this.selector + ' table', Debug.disable);
		...
		_createClass(Paginator, [{
			key: 'getTotalPages',
			value: function getTotalPages(debug) {
			...
			key: 'getCurrentPage',
			value: function getCurrentPage(currentPageSelector) {
				//parameter: todo...
				var pn = this.paginator;
			...
			key: 'backupCurrentPage',
			value: function backupCurrentPage() {
			...
			key: 'gotoPage',
			value: function gotoPage(pageIndex, mode) {
				...
				try {
					...
					pn.children('a').filter(function (index) {
						log('page number: ', (0, _jquery2.default)(this).text());
						return parseInt((0, _jquery2.default)(this).text()) == pageIndex;
					})[0].click();
				} catch (e) {}
			}
		}]);
...
/***/ (function(module, exports) {
	...
	var PostMenu = function () {
		function PostMenu(insertAfterSelector, debug) {
		...
		_createClass(PostMenu, [{
			key: 'addPostMenu',
			value: function addPostMenu(insertAfterSelector) {
			...
			key: 'addDelMenu',
			value: function addDelMenu(menuSelector) {
			...
			key: 'addRepMenu',
			value: function addRepMenu(menuSelector) {
			...
			key: 'addEditMenu',
			value: function addEditMenu(menuContent) {
				...
				$optContent.append($optCat).append($lblPostTitle).append($('</br>')).append($lblCurPrice).append($txtCurPrice).append($lblNewPrice).append($txtNewPrice);
			...
			key: 'addCopyMenu',
			value: function addCopyMenu(menuContent) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //Repost Class
	...
	var Repost = function () {
		function Repost(paginator, debug) {
			_classCallCheck(this, Repost);

			var _$ = this.log = debug === Debug.enable ? console.log : function () {};

			_$("^^^Inside Repost Module");
			this.repostButton = (0, _jquery2.default)('#repost');

			//this.cgTable = new Table('#paginator table', 0);
			this.repostListing = new _Listing2.default();
			this.form = (0, _jquery2.default)("<form></form>");
			this.firstRepostListing = new _Listing2.default();
		...
		_createClass(Repost, [{
			...
			value: function doRepost(debug) {
				...
				listing.from = "doRepost", listing.subject = "backupListing";
				chrome.runtime.sendMessage(listing, function (response) {
					//send out the repost command
					//form object was saved when read the listing
			...
			key: 'getRepostListing',
			value: function getRepostListing(mode, debug) {
				...
				switch (mode) {
					...
					case 1:
						...
						self.firstRepostListing = self.cgTable.readListing(new _Listing2.default());
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //Delete out of date listings
	...
	var Delete = function () {
		function Delete(paginator, debug) {
		...
		_createClass(Delete, [{
			key: 'bindMenuEvents',
			value: function bindMenuEvents() {

				for (var n in arguments[0]) {
					this[n] = (0, _jquery2.default)(arguments[0][n]);
				}
			...
			key: 'updateCurPost',
			value: function updateCurPost(msg, sender, response) {
				...
				if (msg.from = "background", msg.subject = "currentListingUpdated") {
					...
					chrome.runtime.sendMessage({ from: 'clsDelete', subject: 'bg.11.backupPostTitle', PostTitle: postTitle }, function (response) {
			...
			key: 'toggleMenuContent',
			value: function toggleMenuContent(mnuContentSel) {
			...
			value: function changeOption() {
				...
				if (self.optDelCat.is(':checked')) {} else {}
			}
		}, {
			key: 'doOnePost',
			value: function doOnePost() {
			...
			value: function doCatPosts() {
				...
				chrome.runtime.sendMessage({ from: 'clsDelete', subject: 'bg.12.fetchBackupPostTitle' }, function (response) {
					...
					chrome.runtime.sendMessage({ from: 'clsDelete', subject: 'bg.13.backupCatPosts', catPosts: doCatPosts }, function (response) {
			...
			value: function checkStatus() {
				...
				chrome.runtime.sendMessage({ from: 'clsDelete', subject: 'bg.14.fetchCgManagerStatus' }, function (response) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //edit the post fields
		...
		_createClass(Edit, [{
			...
			value: function updateCurPost(msg, sender, response) {
				...
				if (msg.from = "background", msg.subject = "currentListingUpdated") {
					...
					chrome.runtime.sendMessage({ from: 'clsEdit', subject: 'bg.11.backupPostTitle', PostTitle: postTitle }, function (response) {
			...
			value: function changeOption() {
				...
				if (self.optCat.is(':checked')) {} else {}
			}
		}, {
			key: 'doOnePost',
			value: function doOnePost() {
			...
			value: function checkStatus() {
				...
				chrome.runtime.sendMessage({ from: 'clsEdit', subject: 'bg.14.fetchCgManagerStatus' }, function (response) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //Copy existing post , post again

	var _jquery = __webpack_require__(2);

	var _jquery2 = _interopRequireDefault(_jquery);

	var _NewPost = __webpack_require__(14);
		...
		_createClass(Copy, [{
			...
			value: function updateCurPost(msg, sender, response) {
				...
				if (msg.from = "background", msg.subject = "currentListingUpdated") {
					...
					chrome.runtime.sendMessage({ from: 'clsCopy', subject: 'bg.11.backupPostTitle', PostTitle: postTitle }, function (response) {
			...
			value: function checkStatus() {
				...
				chrome.runtime.sendMessage({ from: 'clsCopy', subject: 'fetchCgNewPost' }, function (response) {

					//var newPost = new NewPost();
					if (response.cgNewPost) {
						$$$('=====>clsCopy.checkStatus...post MenuStatus is copy');
						chrome.storage.sync.get(['CategoryID', 'PostingTitle', 'PostalCode', 'Price', 'PostingBody', 'ContactPhone', 'ContactName', 'Street', 'CrossStreet', 'City', 'Region', 'OK4Others'], function (result) {

							$$$('=====>clsCopy.Get the new post fields', result);

							var newPost = new _NewPost2.default();
